...
bench_pred_viz_chicago <- function(
  bm_pred_clean,
  data_key,
  model_key,
  model_subset = c("obliqueRSF", "ranger"),
 ...
 model_levels <- data_smry_overall %>%
  arrange(eval) %>%
  pull(model)
 ...
 diffs <- data_smry %>%
  arrange(data, desc(eval)) %>%
  summarise(winner = model[1],
            adiff = abs(diff(eval)),
            pdiff = perc_diff(eval[1], eval[2]),
            eval = max(eval),
            x = mean(x)) %>%
  mutate(label = table_glue("+{100*adiff} ({pdiff}%)")) %>%
  add_count(winner) %>%
  filter(n == max(n), adiff  > 0.01 | data == 'Overall')
 ...
 xmax <- max(data_smry$x)

 gg_text_header <- tibble(
  x = c(xmax + 1.1, xmax + 1.1),
 ...
 .data_smry <-
  filter(data_smry, data == 'Primary biliary cholangitis; death')

 .diffs <- filter(diffs, data == 'Primary biliary cholangitis; death')

 figs <- list(
  big = ggplot(data_smry) +
   aes(x = x, y = eval) +
  ...
  map(
   ~ .x +
    ...
    scale_y_continuous(limits = c(y_col_1, max(data_smry$eval)*1.15),
                       expand = c(0, 0),
                       breaks = y_breaks,
